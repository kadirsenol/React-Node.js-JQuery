...
import imagebackground from '../assest/image/imgregister.jpg'
import { useNavigate } from 'react-router-dom'
import navbaricon from '../assest/image/iotnavbarblue.png'
...
const registerLoginSchema = Yup.object().shape({
  ...
  ConfirmPassword: Yup.string().required('Password doğrulama alani boş birakilamaz').oneOf([Yup.ref('Password')],'Girilen parolalar eşleşmiyor. !')
})
...
const Register = () => {
  ...
  return (
    <Formik
      initialValues={{ Ad: "", TcNo: "", Email: "", Password: "",ConfirmPassword:"" }}
      ...
      {({values,handleChange,handleSubmit,handleBlur,touched,errors}) => (
        <div style={{backgroundImage:`url(${imagebackground})`, backgroundPosition: 'center', backgroundSize:'cover', backgroundRepeat:'no-repeat'}} className="flex justify-center min-h-screen items-center">
          <img onClick={()=>{navigate("/")}} 
           className="absolute w-10 h-10 top-3 left-5 cursor-pointer hover:bg-zinc-300 rounded-xl" 
           src={navbaricon} 
           alt="ioticon"
           />
          <div className="w-1/4 opacity-1 bg-white bg-opacity-80 rounded-xl">
            ...
            <div className="mb-3 flex justify-center">
              <TextField
                ...
                InputProps={{
                  endAdornment:(
                    <InputAdornment className="cursor-pointer" onClick={()=>{setShowConfirmPassword(!showConfirmPassword)}}>
            ...
            <Button variant="standard" size="small" onClick={handleSubmit}>
              ...
              <Button onClick={()=>(navigate("/Login"))} variant="standard" size="small">
               Go Back Login
              </Button>
            </div>
          </div>
